...
function postResult(txt){
	...
	xmlReq.send("cat_webdir_index="+document.form1.cat_webdir_index.value+"&cat_webdir_title="+document.form1.cat_webdir_title.value+"&cat_webdir_detail="+document.form1.cat_webdir_detail.value+"&action="+document.form1.action.value+"&cat_webdir_id="+document.form1.cat_webdir_id.value);
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    <tr>
        ...
        <td>
        <a href="index.php?page=web_directory&select_page=webdircat_edit_del&action=edit&cat_webdir_id=<?=$rs[cat_webdir_id]?>&cat_webdir_index=<?=$rs[cat_webdir_index]?>" >
        แก้ไข 
        </a>
        &nbsp;
...
<table>
	...
	<tr>
		...
		$result=mysql_query($strSQL);
		while($rs=mysql_fetch_array($result)){
			$name_webcate=$rs['name_webcate'];
		?>
        	<?
			if($cat_webdir_index==$name_webcate){
			?>
        	<option selected><?=$name_webcate?></option>
            <?
			}else{	
			?>
            <option><?=$name_webcate?></option>
            <?
			}
			?>
         <?
